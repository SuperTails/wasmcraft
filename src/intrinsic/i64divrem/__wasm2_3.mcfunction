...
# SetLocalPtr(7)
execute at @e[tag=frameptr] as @e[tag=localptr] run tp @s ~-8 0 1
# PopI64Into(Work(0))
execute at @e[tag=stackptr] as @e[tag=stackptr] run tp @s ~-1 ~ ~
scoreboard players remove %stackptr wasm 1
execute at @e[tag=stackptr] store result score %work%0%hi reg run data get block ~ ~ ~ RecordItem.tag.Memory 1
...
# SetLocalPtr(8)
execute at @e[tag=frameptr] as @e[tag=localptr] run tp @s ~-9 0 1
# PopI32Into(Work(0))
execute at @e[tag=stackptr] as @e[tag=stackptr] run tp @s ~-2 ~ ~
scoreboard players remove %stackptr wasm 2
execute at @e[tag=stackptr] store result score %work%0%lo reg run data get block ~ ~ ~ RecordItem.tag.Memory 1
...
scoreboard players remove %stackptr wasm 2
execute at @e[tag=stackptr] store result score %work%0%lo reg run data get block ~ ~ ~ RecordItem.tag.Memory 1
# BranchIf { t_name: BranchTarget { label: Label { func_idx: CodeFuncIdx(2), idx: 3 }, to_pop: 0, ty: [] }, f_name: BranchTarget { label: Label { func_idx: CodeFuncIdx(2), idx: 4 }, to_pop: 0, ty: [] }, cond: Work(0) }
#   BranchIf { t_name: BranchTarget { label: Label { func_idx: CodeFuncIdx(2), idx: 3 }, to_pop: 0, ty: [] }, f_name: BranchTarget { label: Label { func_idx: CodeFuncIdx(2), idx: 4 }, to_pop: 0, ty: [] }, cond: Work(0) }
...
#   Jump to __wasm2_3
execute unless score %%tempcond reg matches 0..0 run function intrinsic:i64divrem/__wasm2_3
execute at @e[tag=condstackptr] store result score %%tempcond reg run data get block ~-1 ~ ~ RecordItem.tag.Memory 1
#   Branch to __wasm2_4
# RawBranch(Label { func_idx: CodeFuncIdx(2), idx: 4 })
#   Jump to __wasm2_4
execute if score %%tempcond reg matches 0..0 run function intrinsic:i64divrem/__wasm2_4
execute at @e[tag=condstackptr] as @e[tag=condstackptr] run tp @s ~-1 ~ ~