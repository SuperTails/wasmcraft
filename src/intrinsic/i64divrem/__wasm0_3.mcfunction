...
#   Branch to __wasm0_3
# RawBranch(Label { func_idx: CodeFuncIdx(0), idx: 3 })
#   Jump to __wasm0_3
execute unless score %%tempcond reg matches 0..0 run function intrinsic:i64divrem/__wasm0_3
execute at @e[tag=condstackptr] store result score %%tempcond reg run data get block ~-1 ~ ~ RecordItem.tag.Memory 1
#   Branch to __wasm0_4
# RawBranch(Label { func_idx: CodeFuncIdx(0), idx: 4 })
#   Jump to __wasm0_4
execute if score %%tempcond reg matches 0..0 run function intrinsic:i64divrem/__wasm0_4
execute at @e[tag=condstackptr] as @e[tag=condstackptr] run tp @s ~-1 ~ ~