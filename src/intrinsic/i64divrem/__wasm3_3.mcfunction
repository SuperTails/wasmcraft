...
# LoadLocalI64(Work(0, 3), 6)
execute at @e[tag=frameptr] store result score %work%0%lo%3%temp reg run data get block ~-7 ~ ~ RecordItem.tag.Memory 1
execute at @e[tag=frameptr] store result score %work%0%hi%3%temp reg run data get block ~-7 ~ ~1 RecordItem.tag.Memory 1
...
# Comment("I64Shl")
# I64Shl
# SetConst(HalfRegister(Work(1, 3), Lo), 1)
scoreboard players set %work%1%lo%3%temp reg 1
# SetConst(HalfRegister(Work(1, 3), Hi), 0)
scoreboard players set %work%1%hi%3%temp reg 0
# PopI64Into(Work(0, 3))
...
# I64Shl { dst: Work(2, 3), lhs: Work(0, 3), rhs: Work(1, 3) }
...
# I64Const { value: 1 }
# PushI64Const(1)
scoreboard players set %stack%1%lo%3%temp reg 1
scoreboard players set %stack%1%hi%3%temp reg 0
# Comment("LocalGet { local_index: 2 }")
# LocalGet { local_index: 2 }
# LoadLocalI64(Work(0, 3), 2)
execute at @e[tag=frameptr] store result score %work%0%lo%3%temp reg run data get block ~-3 ~ ~ RecordItem.tag.Memory 1
execute at @e[tag=frameptr] store result score %work%0%hi%3%temp reg run data get block ~-3 ~ ~1 RecordItem.tag.Memory 1
# PushI64From(Work(0, 3))
...
# PopI64Into(Work(0, 3))
scoreboard players operation %work%0%lo%3%temp reg = %stack%2%lo%3%temp reg
scoreboard players operation %work%0%hi%3%temp reg = %stack%2%hi%3%temp reg
# StoreLocalI64(Work(0, 3), 7)
execute at @e[tag=frameptr] store result block ~-8 ~ ~ RecordItem.tag.Memory int 1 run scoreboard players get %work%0%lo%3%temp reg
execute at @e[tag=frameptr] store result block ~-8 ~ ~1 RecordItem.tag.Memory int 1 run scoreboard players get %work%0%hi%3%temp reg
# Comment("LocalGet { local_index: 5 }")
# LocalGet { local_index: 5 }
# LoadLocalI64(Work(0, 3), 5)
execute at @e[tag=frameptr] store result score %work%0%lo%3%temp reg run data get block ~-6 ~ ~ RecordItem.tag.Memory 1
execute at @e[tag=frameptr] store result score %work%0%hi%3%temp reg run data get block ~-6 ~ ~1 RecordItem.tag.Memory 1
# PushI64From(Work(0, 3))
scoreboard players operation %stack%2%lo%3%temp reg = %work%0%lo%3%temp reg
scoreboard players operation %stack%2%hi%3%temp reg = %work%0%hi%3%temp reg
# Comment("I64And")
...
scoreboard players operation %work%0%lo%3%temp reg = %stack%1%lo%3%temp reg
scoreboard players operation %work%0%hi%3%temp reg = %stack%1%hi%3%temp reg
# I32Op { dst: HalfRegister(Work(2, 3), Lo), lhs: HalfRegister(Work(0, 3), Lo), op: "&=", rhs: Reg(HalfRegister(Work(1, 3), Lo)) }
scoreboard players operation %param0%0 reg = %work%0%lo%3%temp reg
scoreboard players operation %param1%0 reg = %work%1%lo%3%temp reg
function intrinsic:and
scoreboard players operation %work%2%lo%3%temp reg = %return%0 reg
# I32Op { dst: HalfRegister(Work(2, 3), Hi), lhs: HalfRegister(Work(0, 3), Hi), op: "&=", rhs: Reg(HalfRegister(Work(1, 3), Hi)) }
scoreboard players operation %param0%0 reg = %work%0%hi%3%temp reg
scoreboard players operation %param1%0 reg = %work%1%hi%3%temp reg
function intrinsic:and
scoreboard players operation %work%2%hi%3%temp reg = %return%0 reg
# PushI64From(Work(2, 3))
scoreboard players operation %stack%1%lo%3%temp reg = %work%2%lo%3%temp reg
scoreboard players operation %stack%1%hi%3%temp reg = %work%2%hi%3%temp reg
...
# I64Const { value: 0 }
# PushI64Const(0)
scoreboard players set %stack%2%lo%3%temp reg 0
scoreboard players set %stack%2%hi%3%temp reg 0
# Comment("I64Ne")
# I64Ne
# PopI64Into(Work(1, 3))
scoreboard players operation %work%1%lo%3%temp reg = %stack%2%lo%3%temp reg
...
scoreboard players operation %work%0%lo%3%temp reg = %stack%1%lo%3%temp reg
scoreboard players operation %work%0%hi%3%temp reg = %stack%1%hi%3%temp reg
# I64Eq { dst: Work(2, 3), lhs: Work(0, 3), invert: true, rhs: Work(1, 3) }
scoreboard players set %work%2%lo%3%temp reg 1
execute unless score %work%0%lo%3%temp reg = %work%1%lo%3%temp reg run scoreboard players set %work%2%lo%3%temp reg 0
execute unless score %work%0%hi%3%temp reg = %work%1%hi%3%temp reg run scoreboard players set %work%2%lo%3%temp reg 0
execute store success score %work%2%lo%3%temp reg if score %work%2%lo%3%temp reg matches 0..0
...
scoreboard players operation %work%0%lo%3%temp reg = %stack%1%lo%3%temp reg
scoreboard players operation %work%0%hi%3%temp reg = %stack%1%hi%3%temp reg
# StoreLocalI64(Work(0, 3), 6)
execute at @e[tag=frameptr] store result block ~-7 ~ ~ RecordItem.tag.Memory int 1 run scoreboard players get %work%0%lo%3%temp reg
execute at @e[tag=frameptr] store result block ~-7 ~ ~1 RecordItem.tag.Memory int 1 run scoreboard players get %work%0%hi%3%temp reg
# Comment("I64Const { value: 0 }")
# I64Const { value: 0 }
# PushI64Const(0)
scoreboard players set %stack%1%lo%3%temp reg 0
scoreboard players set %stack%1%hi%3%temp reg 0
# Comment("LocalGet { local_index: 4 }")
# LocalGet { local_index: 4 }
# LoadLocalI64(Work(0, 3), 4)
execute at @e[tag=frameptr] store result score %work%0%lo%3%temp reg run data get block ~-5 ~ ~ RecordItem.tag.Memory 1
execute at @e[tag=frameptr] store result score %work%0%hi%3%temp reg run data get block ~-5 ~ ~1 RecordItem.tag.Memory 1
# PushI64From(Work(0, 3))
scoreboard players operation %stack%2%lo%3%temp reg = %work%0%lo%3%temp reg
scoreboard players operation %stack%2%hi%3%temp reg = %work%0%hi%3%temp reg
# Comment("LocalGet { local_index: 6 }")
...
# I64UComp { dst: Work(2, 3), lhs: Work(0, 3), op: LessThan, rhs: Work(1, 3) }
scoreboard players set %temp%3%lo reg 0
execute if score %work%0%hi%3%temp reg matches ..-1 if score %work%1%hi%3%temp reg matches 0.. run scoreboard players set %temp%3%lo reg 0
execute if score %work%0%hi%3%temp reg matches 0.. if score %work%1%hi%3%temp reg matches ..-1 run scoreboard players set %temp%3%lo reg 1
execute if score %work%0%hi%3%temp reg matches ..-1 if score %work%1%hi%3%temp reg matches ..-1 if score %work%0%hi%3%temp reg < %work%1%hi%3%temp reg run scoreboard players set %temp%3%lo reg 1
execute if score %work%0%hi%3%temp reg matches 0.. if score %work%1%hi%3%temp reg matches 0.. if score %work%0%hi%3%temp reg < %work%1%hi%3%temp reg run scoreboard players set %temp%3%lo reg 1
scoreboard players set %temp%4%lo reg 0
execute if score %work%1%hi%3%temp reg matches ..-1 if score %work%0%hi%3%temp reg matches 0.. run scoreboard players set %temp%4%lo reg 0
execute if score %work%1%hi%3%temp reg matches 0.. if score %work%0%hi%3%temp reg matches ..-1 run scoreboard players set %temp%4%lo reg 1
execute if score %work%1%hi%3%temp reg matches ..-1 if score %work%0%hi%3%temp reg matches ..-1 if score %work%1%hi%3%temp reg < %work%0%hi%3%temp reg run scoreboard players set %temp%4%lo reg 1
execute if score %work%1%hi%3%temp reg matches 0.. if score %work%0%hi%3%temp reg matches 0.. if score %work%1%hi%3%temp reg < %work%0%hi%3%temp reg run scoreboard players set %temp%4%lo reg 1
execute store success score %temp%5%lo reg if score %work%0%hi%3%temp reg = %work%1%hi%3%temp reg
scoreboard players set %temp%6%lo reg 0
execute if score %work%0%lo%3%temp reg matches ..-1 if score %work%1%lo%3%temp reg matches 0.. run scoreboard players set %temp%6%lo reg 0
execute if score %work%0%lo%3%temp reg matches 0.. if score %work%1%lo%3%temp reg matches ..-1 run scoreboard players set %temp%6%lo reg 1
execute if score %work%0%lo%3%temp reg matches ..-1 if score %work%1%lo%3%temp reg matches ..-1 if score %work%0%lo%3%temp reg < %work%1%lo%3%temp reg run scoreboard players set %temp%6%lo reg 1
execute if score %work%0%lo%3%temp reg matches 0.. if score %work%1%lo%3%temp reg matches 0.. if score %work%0%lo%3%temp reg < %work%1%lo%3%temp reg run scoreboard players set %temp%6%lo reg 1
execute if score %temp%3%lo reg matches 1.. run scoreboard players set %work%2%lo%3%temp reg 1
execute if score %temp%4%lo reg matches 1.. run scoreboard players set %work%2%lo%3%temp reg 0
execute if score %temp%5%lo reg matches 1.. run scoreboard players operation %work%2%lo%3%temp reg = %temp%6%lo reg
# PushI32From(Work(2, 3))
...
# StoreLocalI32(Work(0, 3), 8)
execute at @e[tag=frameptr] store result block ~-9 ~ ~ RecordItem.tag.Memory int 1 run scoreboard players get %work%0%lo%3%temp reg
# Comment("Select")
# Select
# PopI32Into(Work(2, 3))
...
# I64Const { value: 0 }
# PushI64Const(0)
scoreboard players set %stack%0%lo%3%temp reg 0
scoreboard players set %stack%0%hi%3%temp reg 0
# Comment("LocalGet { local_index: 7 }")
# LocalGet { local_index: 7 }
# LoadLocalI64(Work(0, 3), 7)
execute at @e[tag=frameptr] store result score %work%0%lo%3%temp reg run data get block ~-8 ~ ~ RecordItem.tag.Memory 1
execute at @e[tag=frameptr] store result score %work%0%hi%3%temp reg run data get block ~-8 ~ ~1 RecordItem.tag.Memory 1
# PushI64From(Work(0, 3))
scoreboard players operation %stack%1%lo%3%temp reg = %work%0%lo%3%temp reg
scoreboard players operation %stack%1%hi%3%temp reg = %work%0%hi%3%temp reg
# Comment("LocalGet { local_index: 8 }")
# LocalGet { local_index: 8 }
# LoadLocalI32(Work(0, 3), 8)
execute at @e[tag=frameptr] store result score %work%0%lo%3%temp reg run data get block ~-9 ~ ~ RecordItem.tag.Memory 1
# PushI32From(Work(0, 3))
scoreboard players operation %stack%2%lo%3%temp reg = %work%0%lo%3%temp reg
# Comment("Select")
# Select
# PopI32Into(Work(2, 3))
scoreboard players operation %work%2%lo%3%temp reg = %stack%2%lo%3%temp reg
# PopI64Into(Work(1, 3))
scoreboard players operation %work%1%lo%3%temp reg = %stack%1%lo%3%temp reg
scoreboard players operation %work%1%hi%3%temp reg = %stack%1%hi%3%temp reg
# PopI64Into(Work(0, 3))
...
scoreboard players operation %work%0%lo%3%temp reg = %stack%0%lo%3%temp reg
scoreboard players operation %work%0%hi%3%temp reg = %stack%0%hi%3%temp reg
# StoreLocalI64(Work(0, 3), 3)
execute at @e[tag=frameptr] store result block ~-4 ~ ~ RecordItem.tag.Memory int 1 run scoreboard players get %work%0%lo%3%temp reg
execute at @e[tag=frameptr] store result block ~-4 ~ ~1 RecordItem.tag.Memory int 1 run scoreboard players get %work%0%hi%3%temp reg
# Comment("LocalGet { local_index: 2 }")
# LocalGet { local_index: 2 }
# LoadLocalI64(Work(0, 3), 2)
execute at @e[tag=frameptr] store result score %work%0%lo%3%temp reg run data get block ~-3 ~ ~ RecordItem.tag.Memory 1
...
# PushI64Const(-1)
scoreboard players set %stack%1%lo%3%temp reg -1
scoreboard players set %stack%1%hi%3%temp reg -1
# Comment("I64Add")
# I64Add
# PopI64Into(Work(1, 3))
scoreboard players operation %work%1%lo%3%temp reg = %stack%1%lo%3%temp reg
...
scoreboard players operation %work%0%lo%3%temp reg = %stack%1%lo%3%temp reg
scoreboard players operation %work%0%hi%3%temp reg = %stack%1%hi%3%temp reg
# StoreLocalI64(Work(0, 3), 2)
execute at @e[tag=frameptr] store result block ~-3 ~ ~ RecordItem.tag.Memory int 1 run scoreboard players get %work%0%lo%3%temp reg
execute at @e[tag=frameptr] store result block ~-3 ~ ~1 RecordItem.tag.Memory int 1 run scoreboard players get %work%0%hi%3%temp reg
# Comment("I64Const { value: -1 }")
# I64Const { value: -1 }
# PushI64Const(-1)
scoreboard players set %stack%1%lo%3%temp reg -1
...
scoreboard players operation %%tempcond reg = %work%0%lo%3%temp reg
#   Branch to __wasm3_3
#   Jump to __wasm3_3
